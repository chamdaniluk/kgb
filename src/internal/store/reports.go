...
func ListIssuedSubmissions(ctx context.Context, pool *pgxpool.Pool, role string, actorUnitID *int64, f IssuedFilter) ([]Submission, error) {
	query := submissionSelect + ` WHERE s.status='terbit'`
	args := []any{}
	n := 1
	if f.Year >= 2000 && f.Year <= 2200 {
		query += ` AND l.issued_at >= make_date($` + itoa(n) + `,1,1) AND l.issued_at < make_date($` + itoa(n) + `+1,1,1)`
		args = append(args, f.Year)
		n++
	}
	if q := strings.TrimSpace(f.Query); q != "" {
		query += ` AND (s.snapshot_name ILIKE $` + itoa(n) + ` OR s.snapshot_nip ILIKE $` + itoa(n) + ` OR l.number ILIKE $` + itoa(n) + ` OR un.name ILIKE $` + itoa(n) + `)`
		args = append(args, "%"+q+"%")
		n++
	}
	if f.UnitID > 0 {
		query += ` AND COALESCE(s.proposed_unit_id, s.snapshot_unit_id, t.unit_id) = $` + itoa(n)
	...
	query += ` ORDER BY l.issued_at DESC NULLS LAST, s.id DESC LIMIT 200`
...
func ListNominations(ctx context.Context, pool *pgxpool.Pool, role string, actorUnitID *int64, f NominationFilter, asOf time.Time) ([]Nomination, error) {
